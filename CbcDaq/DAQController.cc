...
namespace CbcDaq{
	...
	void DAQController::InitialiseGlib(){
		...
		if( fBoardId == "Emulator" ){
			...
			fHwController = new BeController( fBeFirmwareType.c_str() ); 
		}
	...
	void DAQController::ConfigureGlib(){
		...
		Emit( "Message( const char * )", Form( "\tGlibFirmwareType      : %s", fBeFirmwareType.c_str() ) );


		std::string prefix( "file://" );
		try{
			fHwController->ConfigureGlibController( ( prefix + fUhalConfigFileName ).c_str(), fBoardId.c_str(), fBeFirmwareType.c_str() );
		}
		catch( ... ){
		...
		fNFe = fNFe == 1 ? 1 : 2;	
		fNCbc = fHwController->GetGlibSetting( "CBC_expected" );
		if( fBeFirmwareType == "BE_8CBC_v0" ){
			fNCbc = 8;	
		}
	...
	void DAQController::ReadSettingFile( const char *pFname ){
		...
		for( ; cIt != fConfiguration.end(); cIt++ ){
			...
			else if( cType.find( "BeFirmwareType" ) != std::string::npos ){
				fBeFirmwareType = cValue;
