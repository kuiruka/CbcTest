...
void PlotSignalShapeMultiPulseSize( Char_t *pPrefix, Char_t *pTPValues, Char_t *pIpre1, UInt_t pBeId = 0, UInt_t pFeId = 0, UInt_t pCbc = 0, UInt_t pChannel = 0, UInt_t color = 4 ){
	...
	cLegend0->SetHeader( "TP" );
//	cLegend0->SetFillColor(0);


	TString cTPValues( pTPValues );
	...
	if( Strings->GetEntriesFast()){
		...
		while( (os = (TObjString *) iString() ) ){
			...
			TFile *fin = new TFile( Form( "%s%sIpre1%s.root", pPrefix, os->GetString().Data(), pIpre1 ) );
			sscanf( os->GetString().Data(), "%2X", &cTP );
